...
- 예: "서울교통공사"만 등장하면 event_location은 ""입니다.
- 예: "서울역에서 화재 발생"이면 event_location은 "서울역"입니다.
- 예: "서울 지하철 2호선에서 운행 장애 발생"이면 event_location은 "서울 지하철 2호선"입니다.



- event_name은 후속 조치나 결과가 아니라 원인 사건을 기준으로 작성합니다.
- 같은 원인 사건에서 파생된 운행 중단, 운행 재개, 복구, 철거, 점검, 조사, 보고, 대책 기사는 같은 event_name을 사용합니다.
- event_name은 "핵심 장소/대상 + 핵심 원인 사건" 형식으로 작성합니다.
- 장소나 대상이 명확하지 않으면 "핵심 원인 사건"만 작성합니다.
- 운행 중단, 복구 작업, 철거 작업, 안전 점검, 상황 보고, 대책 발표는 원인 사건이 명확할 때 event_name의 중심으로 삼지 않습니다.
- 기사 제목이 후속 조치 중심이어도 본문에 원인 사건이 반복되면 원인 사건을 event_name으로 사용합니다.
...
- event_entities는 핵심 사건의 직접 당사자만 우선 포함합니다.

- event_keywords는 반드시 10개를 출력합니다.
- event_keywords는 같은 사건을 묶기 위한 기준입니다.
- 기사 핵심 사건을 식별할 수 있는 고유명사, 장소, 대상, 원인, 사고 유형, 영향 단어를 우선 포함합니다.
- 후속 조치보다 원인 사건을 식별하는 단어를 우선 포함합니다.
- 너무 일반적인 단어는 제외합니다.
- "서울", "지하철", "공사", "시민", "발표", "논의", "계획", "문제", "상황"처럼 단독으로 사건을 구분하기 어려운 단어는 제외합니다.
- 10개를 채우기 어렵더라도 핵심 사건과 직접 관련된 단어만 사용합니다.

알림 기준:
- alert_topic은 사용자에게 보여줄 짧은 알림 제목입니다.
- alert_message는 사용자에게 보여줄 한 문장 알림 내용입니다.
- alert_topic과 alert_message는 핵심 사건 중심으로 작성합니다.
- 부가 설명이나 낮은 비중의 내용을 알림 제목으로 삼지 마세요.

대상 기준: 
- target_name은 항상 "서울교통공사"로 출력합니다.
- 서울 지하철 운영, 역사, 승객 이동, 열차 운행, 도시철도 서비스와 관련된 기사라면 target_related는 true로 평가할 수 있습니다.
- 서울교통공사가 직접 언급되면 target_mention_type은 direct입니다.
- 서울 지하철, 역사, 열차 운행 등 간접 관련이면 indirect입니다.
- 서울교통공사 및 서울 지하철 운영과 무관한 기사만 none으로 평가합니다.
- 다른 운영기관이 사건 주체인 경우에도 서울 지하철 운영 이미지에 영향을 줄 수 있다면 indirect로 평가할 수 있습니다.
...
- 외부 공사, 타 기관 사고, 자연재해, 외부 단체 시위, 승객 개인 행동 등 외부 요인으로 발생한 사건은 target_responsibility를 0.0~0.3 범위로 낮게 평가합니다.
- 단순히 서울 지하철 운행에 영향이 발생했다는 이유만으로 target_responsibility를 높게 평가하지 마세요.
- 서울교통공사가 직접 운영 실패, 관리 부실, 대응 실패, 시설 문제 등을 일으킨 경우에만 target_responsibility를 0.6 이상으로 평가합니다.
- 서울교통공사가 사고 또는 장애의 직접 원인 제공자인 경우에만 높은 책임도로 평가합니다.
...
- accident: 실제 사고 발생
- labor: 노사갈등, 파업, 임금 협상
- finance: 적자, 무임수송 손실, 재정 부담, 국비 지원 요구
- policy: 법, 제도, 요금, 정책 논의
- crime: 범죄, 고소, 불법행위

- 하나의 사건에 여러 영향이 있으면 결과가 아니라 원인을 기준으로 issue_type을 정합니다.
- 사고, 붕괴, 화재, 충돌, 추락, 부상 위험, 시설 결함이 원인인 경우 safety 또는 accident를 우선 사용합니다.
- 그 결과 운행 중단이나 지연이 발생했더라도 원인이 안전 사고라면 service_disruption을 우선하지 않습니다.
- service_disruption은 안전 원인 없이 운행 지연, 운행 중단, 배차 문제, 시스템 장애가 중심일 때 사용합니다.
...
/*
적용할수도 있는 event name 관련 강화규칙 // event name 클러스터링 비중을 높게 잡아서 더 정교하게 하면 좋음  
- event_name은 기관명보다 사건 자체를 중심으로 작성합니다.
- 같은 사건을 다룬 기사라면 제목 표현이 달라도 같은 event_name이 나오도록 작성합니다.
- 날짜, 수식어, 감정 표현, 언론식 과장 표현은 제거합니다.
- 가능한 형식은 "핵심 대상 + 핵심 사건"으로 작성합니다.
- 예: "서울교통공사, 가족과 만든 사랑의 빵 전달" → "2026년 사랑의 빵 나눔 행사"
- 예: "부정승차 예방 나선 서울교통공사" → "2026년 상반기 부정승차 예방 캠페인"
- 예: "전장연 탑승 시위로 출근길 지연" → "전장연 시위로 인한 지하철 운행 지연"
*/
// 타겟 서울교통공사 고정은 임시 